...
class Register extends CI_Controller 
{
    ...
    function create_account($param1 = '')
    {
        if($param1 == 'teacher')
        {
            ...
            $notify['notify'] = "<strong>".get_phrase('registrar').":</strong>,". " ". get_phrase('reg_teacher') ."<b>".$this->input->post('name')."</b>";
        ...
        if($param1 == 'student')
        {
            ...
            $this->welcome_user($user_id);
             ...
             $notify['notify'] = "<strong>".get_phrase('registrar').":</strong>,". " ". get_phrase('reg_student')."<b>".$this->input->post('name')."</b>";
            $admins = $this->db->get('admin')->result_array();
            foreach($admins as $row)
            {
                $notify['user_id'] = $row['admin_id'];
        ...
        if($param1 == 'parent')
        {
            ...
            $notify['notify'] = "<strong>".get_phrase('registrar').":</strong>,". " ". get_phrase('reg_parent')."<b>".$this->input->post('name')."</b>";
            $admins = $this->db->get('admin')->result_array();
            foreach($admins as $row)
            {
                $notify['user_id'] = $row['admin_id'];
